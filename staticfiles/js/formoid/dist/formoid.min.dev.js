...
jQuery(function ($) {
  var Formoid = function () {
    ...
    var attachMethods = function attachMethods(obj, context, methods) {
    ...
    _Form.prototype.recaptcha_token = function (value) {
    ...
    _Form.prototype.getCaptchaToken = function () {
    ...
    _Form.prototype.send = function (data, beforeStart) {
      var _this = this;
      ...
      recaptchaToken.then(function (token) {
        ...
        $ajax(API_URL, {
          ...
          try {
            ...
            if (data.error) defer.reject(data.error);else defer.resolve(data.response);
          } catch (e) {
            defer.reject('Incorrect server response.');
          }
        }).fail(function () {
          var error = 'Failed to query the server. ';
          if ('onLine' in navigator && !navigator.onLine) error += 'No connection to the Internet.';else error += 'Check the connection and try again.';
          defer.reject(error);
        });
      });
      return defer;
  ...
  $('[data-form-type="formoid"]').each(function () {
    var form,
        ...
        $captcha = $this.find('[data-form-captcha]'); // on change form inputs if input[type=file].files[0]size>1mb disable sumbit button and show form alert
    ...
    $form.submit(function (event) {
      ...
      function parseInputs(inputs) {
        ...
        inputs.each(function (index, input) {
          ...
          arrDeferred.push(objDeferred); // valide email
      ...
      parseInputs(inputs).then(function (arrDef) {
        $.when.apply(this, arrDef).then(function () {
          form.send(data).always(function () {
            $submit.removeClass('btn-loading').prop('disabled', false);
            $alert.removeAttr('hidden');
