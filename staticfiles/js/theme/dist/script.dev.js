...
(function ($) {
  ...
  $(function () {
    $('html').addClass($.isMobile() ? 'mobile' : 'desktop'); // .mbr-navbar--sticky
    ...
    function videoParser(card) {
      $(card).outerFind('[data-bg-video]').each(function () {
        ...
        $('> *:eq(0)', this).before($img); // youtube or vimeo
    ...
    $(window).resize().scroll(); // smooth scroll
          ...
          do {
            if (target.hash) {
              ...
              $(useBody ? 'body' : target.hash).each(function () {
                e.preventDefault(); // in css sticky navbar has height 64px
                // var stickyMenuHeight = $('.mbr-navbar--sticky').length ? 64 : 0;

                var stickyMenuHeight = $(target).parents().hasClass('navbar-fixed-top') ? 60 : 0;
                var goTo = target.hash == '#bottom' ? $(this).height() - $(window).height() : $(this).offset().top - stickyMenuHeight; // Disable Accordion's and Tab's scroll
  ...
  if (!isBuilder) {
    ...
    $(document).ready(function () {
      ...
      if ($.isMobile()) {
        return; // enable animation on scroll
        ...
        var elCarouselItem = function elCarouselItem(element) {
        ...
        var checkIfInView = function checkIfInView() {
          ...
          $.each($animatedElements, function () {
            ...
            var element_bottom_position = element_top_position + element_height; // check to see if this current element is within viewport

            if ((element_bottom_position >= window_top_position && element_top_position - 50 <= window_bottom_position || elCarouselItem($element)) && $element.hasClass('hidden')) {
              $element.removeClass('hidden').addClass('animate__fadeInUp animate__delay-1s').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function () {
        ...
        var $animatedElements = $('p, h1, h2, h3, h4, h5, a, button, small, img, li, blockquote, .mbr-author-name, em, label, input, select, textarea, .input-group, .form-control, .iconbox, .btn-social, .mbr-figure, .mbr-map, .mbr-testimonial .card-block, .mbr-price-value, .mbr-price-figure, .dataTable, .dataTables_info').not(function () {
  ...
  if (!isBuilder) {
    ...
    if (!$.isMobile()) {
      var $menu = $('section.menu'),
          $width = $(window).width(),
          $collapsed = $menu.find('.navbar').hasClass('collapsed'); // check if collapsed on
    ...
    $(document).on('add.cards', function (event) {
      if ($(event.target).find('.form-with-styler').length) {
        ...
        $(form).find('input[type=date]').each(function () {
          if ($(this).datetimepicker) $(this).datetimepicker({
            format: 'Y-m-d',
            timepicker: false
          });
        });
