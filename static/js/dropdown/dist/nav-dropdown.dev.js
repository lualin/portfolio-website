...
(function ($) {
  ...
  var $$ = function () {
    ...
    Item.prototype.parent = function () {
      return new Item($(this).map(function () {
        var $$this = new Item(this);
        if ($$this.is(':upper')) return null;
        return $($$this.is(':toggle') ? this.parentNode.parentNode : this).closest(Selector.DROPDOWN).find('>' + Selector.DATA_TOGGLE)[0];
      }), this);
    };
    ...
    Item.prototype.children = function (deepSearch) {
      ...
      $(this).each(function () {
        ...
        $items.each(function () {
          if (deepSearch && !this.offsetWidth && !this.offsetHeight || this.disabled || $(this).is(Selector.DATA_BUTTON) || $(this).hasClass(ClassName.DISABLED) || ~$.inArray(this, elements)) {
            return;
          }
    ...
    Item.prototype.root = function () {
      return new Item($(this).closest(Selector.BASE), this);
    };
    ...
    Item.prototype.is = function (selector) {
      ...
      while (selector = selectors.pop()) {
        switch (selector) {
          case ':root':
            if (!$this.is(Selector.BASE)) return false;
            break;

          case ':upper':
            if (!$this.parent().parent().is(Selector.BASE)) return false;
            break;

          case ':opened':
          case ':closed':
            if (selector == ':opened' != $this.parent().hasClass(ClassName.OPEN)) return false;

          case ':toggle':
            if (!$this.is(Selector.DATA_TOGGLE)) return false;
            break;

          default:
            if (!this.props[selector]) return false;
            break;
        }
      }
  ...
  var NavDropdown = function NavDropdown(element) {
  ...
  NavDropdown.prototype.toggle = function (event) {
    ...
    NavDropdown._clearMenus($.Event('click', {
      target: this,
      data: {
  ...
  NavDropdown._clearMenus = function (event) {
    ...
    if (event.target) {
      if (this === document) {
        if ($(event.target).is('a:not([disabled], .' + ClassName.DISABLED + ')')) {
          collapseEvent = $.Event(Event.COLLAPSE, {
    ...
    for (var i = 0; i < toggles.length; i++) {
      ...
      if (event.type === 'click' && /input|textarea/i.test(event.target.tagName) && $.contains(parent, event.target)) {
        continue;
      }
  ...
  NavDropdown._dataApiKeydownHandler = function (event) {
    ...
    if (event.which == Hotkeys.ESC) {
      ...
      var toggle = $(event.target).parents(Selector.DROPDOWN + '.' + ClassName.OPEN).last().find('>' + Selector.DATA_TOGGLE);
  ...
  NavDropdown._isCollapsed = function (rootNode) {
    ...
    return rootNode && (match = /navbar-toggleable-(xs|sm|md|lg|xl)/.exec(rootNode.className)) && window.innerWidth < Breakpoints[match[1].toUpperCase()];
  }; // static
  ...
  NavDropdown._dataApiResizeHandler = function () {
    $(Selector.BASE).each(function () {
      ...
      if (isCollapsed) {
        ...
        $(this).removeClass(ClassName.SM); // $(this).removeClass(ClassName.SM + ' in'); /// ???
        // NavDropdown._clearMenus();
      }
    });
  };
  ...
  $.fn[NAME].$$ = $$;
  $(window).on(Event.RESIZE_DATA_API + ' ' + Event.LOAD_DATA_API, NavDropdown._dataApiResizeHandler);
  $(document).on(Event.KEYDOWN_DATA_API, Selector.BASE, NavDropdown._dataApiKeydownHandler).on(Event.NAVBAR_COLLAPSE, NavDropdown._clearMenus).on(Event.CLICK_DATA_API, NavDropdown._clearMenus).on(Event.CLICK_DATA_API, Selector.DATA_TOGGLE, NavDropdown.prototype.toggle).on(Event.CLICK_DATA_API, Selector.FORM_CHILD, function (e) {
    e.stopPropagation();
  });
  $(window).trigger(Event.READY);
})(jQuery);